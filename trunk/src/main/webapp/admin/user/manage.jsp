...
<script type="text/javascript" charset="utf-8">
	$(function() {
		$('#admin_user_manage_dataGrid').datagrid(
				{
	...
	function searchFun(){
		$('#admin_user_manage_dataGrid').datagrid('load',$('#admin_user_manage_searchForm').serializeObject());
	}
						...
						<form id="admin_user_manage_searchForm">
							<table>
								<tr>
									...
									<td><input class="Wdate" name="startTime" placeholder="点击选择时间" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly" />-<input class="Wdate" name="endTime" placeholder="点击选择时间" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" readonly="readonly" /></td>
									<td><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true" onclick="searchFun();">查找</a></td>
									<td><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-undo',plain:true" onclick="resetSearch();">重置</a></td>
								</tr>
							</table>
						</form>
					</td>
